...
fun RegisterScreen(
    ...
    Scaffold(containerColor = AppBackground) { padding ->
        Column(
            ...
            Spacer(Modifier.height(32.dp))
            Text("Create account", color = TextPrimary, style = MaterialTheme.typography.headlineLarge)
            Spacer(Modifier.height(24.dp))

            Text("Display name", color = TextMuted, style = MaterialTheme.typography.bodyLarge)
            Spacer(Modifier.height(8.dp))
            HubTextField(value = displayName, onValueChange = onDisplayNameChange, placeholder = "")
            ...
            HubPrimaryButton(label = "Create account", onClick = onCreateAccount, modifier = Modifier.fillMaxWidth())
